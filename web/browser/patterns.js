...
export class PatternTracker {
  ...
  gap() {
    // Preserve recurrence and event de-duplication in the same recording,
    // while requiring a new observed train after any unseen interval.
    const { memory, seen, context, lastEnd } = this;
    this.reset({ keepBaseline: true });
    Object.assign(this, { memory, seen, context, lastEnd, afterGap: true });
    this.value.recurrence = memory;
  }
  update(frame) {
    ...
    if (frame.mixed || frame.settingsUncertain) {
      this.reset();
      return this.value;
    }
    ...
    if (this.lastEnd != null && frame.start - this.lastEnd > 0.1) this.gap();
    this.context = context;
    if (this.afterGap && this.lastEnd != null)
      this.memory *= Math.exp(-Math.max(0, frame.start - this.lastEnd) / 18);
    const dt =
      this.lastEnd == null || this.afterGap
