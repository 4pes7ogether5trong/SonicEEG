...
export function showcasePhase(time, slot = 0) {
  ...
  if (slot === 2 && time % 120 >= 18 && time % 120 < 36)
    return 'N3-like sleep · rhythmic slow-wave interval';
...
function potential(pos, t, slot, seed) {
  ...
  if (slot === 0) {
    ...
    const phaseTime = t % 120;
    const rhythmic = smooth((phaseTime - 18) / 2) * smooth((36 - phaseTime) / 2);
    value =
      0.3 * value +
      waxing *
        (0.35 + front) *
        (100 * Math.sin(TAU * 0.87 * t + 0.25 * x) +
          (55 - 45 * rhythmic) * Math.sin(TAU * 1.43 * t + 0.6 * z + phase));
