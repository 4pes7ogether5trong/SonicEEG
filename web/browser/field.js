  ...
  drawSoftware() {
      ...
      if (o.fluid) {
        ...
        const positions = grid.vertices.map((v, i) => {
          ...
          for (const d of ds.filter((d) => d.active).slice(0, 24)) {
            ...
            bulge += 0.075 * d.size * Math.exp((-angle * angle) / 0.035);
          }
        ...
        for (const d of ds) {
          ...
          const normal = normalize([p[0], p[1] / 0.88, p[2] / 1.12]);
          const height = d.active ? waterAt(p, cs, o).height + 0.075 * d.size : 0;
          const at = (y) =>
            project(
              base.map(
                (x, j) =>
                  (x * dp.radial + normal[j] * (height + y * d.size)) * dp.scale +
                  (j === 0 ? dp.offset : 0),
              ),
            );
          for (const part of d.parts) {
            const point = at(part.y);
            this.softwareDrops.push({
              ...
              radius: Math.max(
                1,
                Math.min(w / (o.mode === 'side' ? 10 : 6), h / 4) *
                  this.zoom *
                  part.radius *
                  d.size *
                  dp.scale,
    ...
    this.softwareDrops
      ...
      .forEach((d) => {
        ...
        ctx.beginPath();
        if (d.shape === 'stem') {
          const dx = d.tip[0] - d.base[0],
            dy = d.tip[1] - d.base[1],
            length = Math.hypot(dx, dy) || 1;
          const sx = (-dy / length) * d.radius,
            sy = (dx / length) * d.radius;
          ctx.moveTo(d.base[0] + sx, d.base[1] + sy);
          ctx.lineTo(d.tip[0], d.tip[1]);
          ctx.lineTo(d.base[0] - sx, d.base[1] - sy);
          ctx.closePath();
        } else {
