...
// Auxiliary display rows retain their geometry but do not map to the scalp.
export function auxiliaryChannel(input) {
  const name = String(input).toUpperCase().replace(/[–—−]/g, '-').replace(/\s+/g, '');
  if (/^(?:SKIP|ECG\d*|EKG\d*|EMG\d*|EOG\d*|RESP|PHOTIC|SPO2|PR|A[12]-E[12])$/.test(name))
    return { name, ignore: true };
  if (/^(?:ECG|EKG)-?(?:LA|RA|LL|RL)(?:-(?:ECG|EKG)-?(?:LA|RA|LL|RL))?$/.test(name))
    return { name: 'ECG', ignore: true };
  return null;
}
const chain = (s) =>
  s
...
export const TEMPLATES = [
  { name: 'Longitudinal bipolar', channels: LB.flat(), chains: LB },
  { name: 'Transverse bipolar', channels: TB.flat(), chains: TB },
];
...
export function recognizeMontage(labels) {
  ...
  const best = candidates[0],
    next = candidates[1];
  const match =
    names.length >= 4 && best.matched / names.length >= 0.9 && best.matched > next.matched;
  return {
    ...
    alternatives: candidates.filter((c) => c.matched === best.matched).map((c) => c.name),
  };
}
export function parseSettings(text) {
  ...
  const notch = s.match(/notch\s*[:=]?\s*(off|on|50|60)(?:\s*Hz)?/i)?.[1]?.toLowerCase() ?? null;
  const seconds = value(/(\d+(?:\.\d+)?)\s*(?:s|sec|seconds)\s*\/\s*(?:page|screen)/i);
...
export function derive(target, channels) {
  ...
  while (queue.length) {
    ...
    if (node === d.b) {
      ...
      return {
        ...meta,
        labelInferred: path.some((e) => e.c.labelInferred),
        name: d.name,
        samples,
        status: 'derived',
