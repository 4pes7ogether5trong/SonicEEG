...
export function missingWaterSources(frame, selected = '') {
  const measured = new Set(
    (frame.waveform?.channels || [])
      .filter((c) => c.valid && c.status === 'observed')
      .map((c) => c.name),
  );
  return (frame.channels || [])
    .filter(
      (c) => c.status !== 'derived' && (!selected || c.name === selected) && !measured.has(c.name),
    )
    .flatMap((c) => {
      const d = parseDerivation(c.name);
      return d
        ? [normalize(POSITIONS[d.a]), ...(d.bipolar ? [normalize(POSITIONS[d.b])] : [])]
        : [];
    })
    .slice(0, 64);
}
export function missingWaterAt(p, sources, coverage = 0) {
  let missing = 0;
  for (const center of sources) {
    const d = Math.acos(
      clamp(
    ...
    if (d < RIPPLE_EXTENT) missing = Math.max(missing, (1 - (d / RIPPLE_EXTENT) ** 2) ** 3);
  }
  return coverage < 0.015 || missing > coverage * 0.55;
}
