...
export function retainWaveform(samples, rate, { center = true } = {}) {
  if (!Number.isFinite(rate) || rate <= 0 || samples.length < 2 || !samples.every(Number.isFinite))
    return null;
  const n = samples.length,
    mean = center ? samples.reduce((s, v) => s + v, 0) / n : 0;
  let indices;
...
export function waveformSnapshot(rows, channels, start, rate, options) {
  const kept = channels.flatMap((c, i) => {
    const wave =
      c.valid && c.status !== 'expected' ? retainWaveform(rows[i].samples, rate, options) : null;
    return wave
