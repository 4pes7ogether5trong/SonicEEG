...
The tracker retains 12 seconds of candidate times. Three recent events can establish repetition; a gap over 3 seconds starts a new sequence. Median inter-event spacing supplies a descriptive repetition rate and a separate regularity value. Persistence is elapsed time while repetition remains recent, or a pinned spectral deviation is present. Slow recurrence memory rises with a 10-second time constant and decays with an 18-second constant. Desired emphasis is `0.7 × (1 − exp(−persistence/10)) + 0.3 × recurrence`, capped to [0,1], with a 6-second release when it falls. A single transient is immediately audible without having to build persistence. Recent recurring bursts can retain emphasis, but quiet breaks reset the continuous-persistence duration. Irregular sharp candidates and artifacts can also contribute: the sound is an attention aid, not an event label.
